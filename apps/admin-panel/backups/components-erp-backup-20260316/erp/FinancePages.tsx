...
import { ErpBadge, ErpButton, ErpSelect, ErpTableHeader, ErpTableCell, ErpCard, Row, STitle, ErpKpiCard, Table, TableBody, TableHeader, TableRow } from "@/components/erp/ui";
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <ErpButton colorClass="muted-foreground" ghost>匯出帳款表</ErpButton>
        </div>
      </div>
      ...
      <ErpCard>
        <div className="overflow-x-auto">
          <Table>
            <TableHeader><TableRow><ErpTableHeader>發票號</ErpTableHeader><ErpTableHeader>供應商</ErpTableHeader><ErpTableHeader>關聯 PO</ErpTableHeader><ErpTableHeader>金額</ErpTableHeader><ErpTableHeader>付款到期</ErpTableHeader><ErpTableHeader>狀態</ErpTableHeader><ErpTableHeader>操作</ErpTableHeader></TableRow></TableHeader>
            <TableBody>
              {AP.map((r, i) => (
                <TableRow key={i} className="hover:bg-erp-blue/[0.04] transition-colors">
                  ...
                  <ErpTableCell><ErpBadge colorClass={r.status === "已付款" ? "erp-green" : r.status === "核對中" ? "erp-yellow" : r.status === "逾期" ? "erp-red" : "erp-orange"} label={r.status} /></ErpTableCell>
                  <ErpTableCell>
                    <Row className="gap-1.5">
                      {r.status === "待付款" && <ErpButton colorClass="erp-finance">核對發票</ErpButton>}
                      {r.status === "核對中" && <ErpButton colorClass="erp-green">確認付款</ErpButton>}
                      {r.status === "逾期" && <ErpButton colorClass="erp-red">緊急付款</ErpButton>}
                    </Row>
                  </ErpTableCell>
                </TableRow>
              ))}
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <ErpButton colorClass="erp-finance">🔄 手動觸發即時同步</ErpButton>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <ErpCard>
          ...
          <div className="overflow-x-auto">
            <Table>
              <TableHeader><TableRow><ErpTableHeader>商品</ErpTableHeader><ErpTableHeader>銷量</ErpTableHeader><ErpTableHeader>毛利/件</ErpTableHeader><ErpTableHeader>毛利率</ErpTableHeader></TableRow></TableHeader>
              <TableBody>
                {[
