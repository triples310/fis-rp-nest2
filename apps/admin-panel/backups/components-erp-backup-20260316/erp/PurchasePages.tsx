...
import { ErpBadge, ErpButton, ErpInput, ErpSelect, ErpTableHeader, ErpTableCell, ErpCard, Row, STitle, ErpModal, Divider, Table, TableBody, TableHeader, TableRow } from "@/components/erp/ui";
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
        ...
        <ErpButton colorClass="erp-yellow" onClick={() => setShowCreate(true)}>＋ 建立採購單</ErpButton>
      </div>
      ...
      <ErpCard>
        <div className="overflow-x-auto">
          <Table>
            <TableHeader><TableRow><ErpTableHeader>採購單號</ErpTableHeader><ErpTableHeader>供應商</ErpTableHeader><ErpTableHeader>狀態</ErpTableHeader><ErpTableHeader>品項</ErpTableHeader><ErpTableHeader>金額</ErpTableHeader><ErpTableHeader>建立日期</ErpTableHeader><ErpTableHeader>預計到貨</ErpTableHeader><ErpTableHeader>操作</ErpTableHeader></TableRow></TableHeader>
            <TableBody>
              {POS.map((po) => (
                <TableRow key={po.id} className="hover:bg-erp-yellow/[0.04] transition-colors">
                  ...
                  <ErpTableCell><ErpBadge colorClass={po.status === "待驗收" ? "erp-orange" : po.status === "已入庫" ? "erp-green" : "muted-foreground"} label={po.status} /></ErpTableCell>
                  ...
                  <ErpTableCell>
                    <Row className="gap-1.5">
                      <ErpButton colorClass="muted-foreground" ghost onClick={() => setDetail(po)}>查看</ErpButton>
                      {po.status === "待驗收" && <ErpButton colorClass="erp-orange">執行驗收</ErpButton>}
                      {po.status === "草稿" && <ErpButton colorClass="erp-yellow">提交</ErpButton>}
                    </Row>
                  </ErpTableCell>
                </TableRow>
              ))}
      ...
      {detail && (
        <ErpModal title={`採購單 · ${detail.id}`} onClose={() => setDetail(null)}>
          ...
          <div className="overflow-x-auto">
            <Table>
              <TableHeader><TableRow><ErpTableHeader>商品</ErpTableHeader><ErpTableHeader>數量</ErpTableHeader><ErpTableHeader>單價</ErpTableHeader><ErpTableHeader>小計</ErpTableHeader></TableRow></TableHeader>
              <TableBody>
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            return (
              <ErpCard key={i} className="mb-3">
                <Row className="justify-between mb-2.5 flex-wrap gap-2">
                  ...
                  {d && <ErpBadge colorClass={d === "pass" ? "erp-green" : "erp-red"} label={d === "pass" ? "✓ 允收" : "✗ 退貨"} />}
                </Row>
                {!d && (
                  <div>
                    ...
                    <div className="flex gap-2">
                      <ErpButton colorClass="erp-green" onClick={() => setDecisions((p) => ({ ...p, [i]: "pass" }))} className="flex-1">✓ 允收入庫</ErpButton>
                      <ErpButton colorClass="erp-red" onClick={() => setDecisions((p) => ({ ...p, [i]: "fail" }))} className="flex-1">✗ 退貨</ErpButton>
                    </div>
                  </div>
                )}
        ...
        <ErpCard>
          ...
          <div className="overflow-x-auto">
            <Table>
              <TableHeader><TableRow><ErpTableHeader>日期</ErpTableHeader><ErpTableHeader>PO</ErpTableHeader><ErpTableHeader>商品</ErpTableHeader><ErpTableHeader>結果</ErpTableHeader><ErpTableHeader>QC</ErpTableHeader></TableRow></TableHeader>
              <TableBody>
                {[
                  ...
                  <TableRow key={i}>
                    ...
                    <ErpTableCell><ErpBadge colorClass={r.ok ? "erp-green" : "erp-red"} label={r.ok ? "允收" : "退貨"} /></ErpTableCell>
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
        ...
        <ErpButton colorClass="erp-yellow">一鍵建立所有緊急 PO</ErpButton>
      </div>
      <ErpCard>
        <div className="overflow-x-auto">
          <Table>
            <TableHeader><TableRow><ErpTableHeader>原料</ErpTableHeader><ErpTableHeader>現有庫存</ErpTableHeader><ErpTableHeader>安全庫存</ErpTableHeader><ErpTableHeader>在途數量</ErpTableHeader><ErpTableHeader>預計需求(30天)</ErpTableHeader><ErpTableHeader>建議採購量</ErpTableHeader><ErpTableHeader>優先級</ErpTableHeader><ErpTableHeader>操作</ErpTableHeader></TableRow></TableHeader>
            <TableBody>
              {[
                ...
                <TableRow key={i} className="hover:bg-erp-yellow/[0.04] transition-colors">
                  ...
                  <ErpTableCell>{r.urgent ? <ErpBadge colorClass="erp-red" label="🔴 緊急" /> : <ErpBadge colorClass="erp-green" label="正常" />}</ErpTableCell>
                  <ErpTableCell>{r.suggest > 0 && <ErpButton colorClass="erp-yellow" className="text-[10px] py-1 px-2.5">建立 PO</ErpButton>}</ErpTableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
        ...
        <ErpButton colorClass="erp-yellow">＋ 新增供應商</ErpButton>
      </div>
      <ErpCard>
        <div className="overflow-x-auto">
          <Table>
            <TableHeader><TableRow><ErpTableHeader>供應商</ErpTableHeader><ErpTableHeader>供應品類</ErpTableHeader><ErpTableHeader>付款天期</ErpTableHeader><ErpTableHeader>聯絡人</ErpTableHeader><ErpTableHeader>QC合格率</ErpTableHeader><ErpTableHeader>訂單數</ErpTableHeader><ErpTableHeader>評分</ErpTableHeader><ErpTableHeader>操作</ErpTableHeader></TableRow></TableHeader>
            <TableBody>
              {VENDORS.map((v, i) => (
                <TableRow key={i} className="hover:bg-erp-yellow/[0.04] transition-colors">
                  ...
                  <ErpTableCell><Row className="gap-1.5"><ErpButton colorClass="muted-foreground" ghost>編輯</ErpButton><ErpButton colorClass="erp-yellow" ghost>下單</ErpButton></Row></ErpTableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
