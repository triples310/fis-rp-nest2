  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
        ...
        <Button variant="erp-purple-ghost">匯出報表</Button>
      </div>
      ...
      <ErpCard>
        <div className="overflow-x-auto">
          <Table>
            <ShadTableHeader><TableRow><ErpTableHeader>批號</ErpTableHeader><ErpTableHeader>商品</ErpTableHeader><ErpTableHeader>效期</ErpTableHeader><ErpTableHeader>入庫日</ErpTableHeader><ErpTableHeader>倉位</ErpTableHeader><ErpTableHeader>總量</ErpTableHeader><ErpTableHeader>鎖定</ErpTableHeader><ErpTableHeader>可用</ErpTableHeader><ErpTableHeader>狀態</ErpTableHeader></TableRow></ShadTableHeader>
            <TableBody>
              {list.map((b) => (
                <TableRow key={b.batch} className="hover:bg-erp-purple/[0.04] transition-colors">
                  ...
                  <ErpTableCell>
                    <Badge
                      variant={b.days <= 0 ? "erp-red" : b.days <= 7 ? "erp-red" : b.days <= 30 ? "erp-yellow" : "erp-green" as any}
                    >
                      {b.days <= 0 ? "已過期" : b.days <= 7 ? "緊急即期" : b.days <= 30 ? "即期品" : "正常"}
                    </Badge>
                  </ErpTableCell>
                </TableRow>
              ))}
    ...
    <div>
      ...
      <ErpCard className="mb-4">
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <Button variant="erp-purple" onClick={() => setTraced(true)}>🔍 追蹤</Button>
        </div>
      </ErpCard>
      {traced && (
          ...
          <ErpCard>
            ...
            {[
              ...
              <div key={i} className="flex items-center gap-2.5 p-3 bg-background rounded-md mb-2 border border-border">
                <Badge variant={r.colorClass as any}>{r.type}</Badge>
                <span className="font-mono text-xs text-foreground flex-1">{r.id}</span>
                <Button variant={`${r.colorClass}-ghost` as any} className="text-[10px] py-1 px-2.5">查看</Button>
              </div>
            ))}
          </ErpCard>
        </div>
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
        ...
        <Badge variant="erp-red">{`${b.length} 批需注意`}</Badge>
      </div>
      <ErpCard>
        <div className="overflow-x-auto">
          <Table>
            <ShadTableHeader><TableRow><ErpTableHeader>批號</ErpTableHeader><ErpTableHeader>商品</ErpTableHeader><ErpTableHeader>倉位</ErpTableHeader><ErpTableHeader>剩餘天數</ErpTableHeader><ErpTableHeader>可用量</ErpTableHeader><ErpTableHeader>效期</ErpTableHeader></TableRow></ShadTableHeader>
            <TableBody>
              {b.map((x, i) => (
                <TableRow key={i}>
